...
- (void)initMinkasu2FA:(nonnull NSString *)minkasu2FAConfig{
    if (minkasu2FAConfig && minkasu2FAConfig.length > 0 && ![minkasu2FAConfig isEqualToString:@"minkasuPayByMethod"]) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.isSkipInit) {
            ...
            @try {
                ...
                self->_onMinkasu2FAInit(@{@"status":status,@"initType":INIT_BY_METHOD,@"errorMessage":exception.reason,@"errorCode":exception.name});
            }
        }
    });
}
...
- (void)setMinkasu2FAConfig:(NSString *)minkasu2FAConfig{
    if(minkasu2FAConfig && minkasu2FAConfig.length > 0){
        _minkasu2FAConfig = minkasu2FAConfig;
        if(_webView){
...
-(void)onPayByAttribute{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        Minkasu2FAConfig *config = [self createConfig:_minkasu2FAConfig];
        NSString *status = STATUS_SUCCESS;
        NSError *err = nil;
        if (self.isSkipInit) {
            ...
            @try {
                ...
                status = STATUS_FAILURE;
                _onMinkasu2FAInit(@{@"status":status,@"initType":INIT_BY_ATTRIBUTE,@"errorMessage":exception.reason,@"errorCode":exception.name});
            }
        }
    });
}
...
- (void)minkasu2FACallback:(Minkasu2FACallbackInfo *)minkasu2FACallbackInfo{
    NSError *error;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:minkasu2FACallbackInfo.data
    ...
    _onMinkasu2FAResult(@{@"infoType":@(minkasu2FACallbackInfo.infoType),@"data":dataStr});
}
