...
class Minkasu2FAUtil {
    ...
    void performMinkasu2faOperation(String merchantCustomerId, String operationTypeStr) throws Exception {
        ...
        if (mContext.getCurrentActivity() != null) {
            ...
            try {
                Minkasu2faSDK minkasu2faSDKInstance = Minkasu2faSDK.initReactSDKOperation(mContext.getCurrentActivity(), operationType, merchantCustomerId, RCT_MINKASU_2FA_SDK_VERSION, minkasu2faCallbackInfo -> Log.e("Minkasu2FAOperation", minkasu2faCallbackInfo.getData() != null ? minkasu2faCallbackInfo.getData().toString() : "No operation result"));
                    ...
                    if (Minkasu2faSDK.isSupportedPlatform()) {
                        ...
                        if (configJson.has(CUSTOMER_ORDER_INFO)) {
                            ...
                            if (!JSONObject.NULL.equals(orderInfo) && orderInfo.length() > 0) {
                                ...
                                order.setCustomData(getJsonStringValue(orderInfo, ORDER_DETAILS, null));
                            }
                        }
                        ...
                        Minkasu2faSDK.initReactSDK(mActivity, configObj, view, RCT_MINKASU_2FA_SDK_VERSION, info -> {
